...
//cipher
function algorithmCipher(text, shift) {
  //tests
...
const word = algorithmCipher('adrian', 4); // output 'ehvmer'
console.log(`Szyfrowanie: ${word}`);



//decipher
function algorithmDecipher(text, shift) {
  //tests
  if (!text) throw new Error('Missing argument: text');
  if (!shift) throw new Error('Missing argument: shift');
...
const wordDe = algorithmDecipher('ehvmer', 4); // output 'adrian'
console.log(`Odszyfrowanie: ${wordDe}`);
